    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('buku.update', $book->id) }}" method="post">
                ...
                <select name="id_lokasi" class="form-select">
                    ...
                    @foreach ($locations as $item)
                        <option {{ $item->id == $book->id_lokasi ? 'selected' : '' }} value="{{ $item->id }}">
                            {{ $item->kode_lokasi . '-' . $item->label . '-' . $item->rak }}</option>
                    @endforeach
                </select>
                ...
                <input type="text" class="form-control" value="{{ $book->judul }}" name="judul">
                ...
                <textarea class="form-control" name="keterangan" cols="30" rows="5">{{ $book->keterangan }}</textarea>
